...
module four_cross_three_bit_multiplier(input [3:0]b, input [2:0]a, output [5:0] c, output carry);
  assign c[0]=a[0]&b[0];
  wire [3:0]firstsum;
  wire firstcarry;
  four_bit_adder one(.a({1'b0, b[3]&a[0], b[2]&a[0], b[1]&a[0]}),
                     .b({b[3]&a[1], b[2]&a[1], b[1]&a[1], b[0]&a[1]}), .cin(1'b0), 
                     .finalsum(firstsum), .cout(firstcarry));
  assign c[1]=firstsum[0];
  
  four_bit_adder two(.a({b[3]&a[2], b[2]&a[2], b[1]&a[2], b[0]&a[2]}),
                     .b({firstcarry, firstsum[3], firstsum[2], firstsum[1]}), .cin(1'b0), 
                     .finalsum(c[5:2]), .cout(carry));
